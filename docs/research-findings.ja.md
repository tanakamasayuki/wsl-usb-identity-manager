...
→ **仕様要件**: プローブ実行前に `Service` を読み、WinUSB でない場合は
「Zadig で WinUSB を割り当ててください」と案内する。
自動でドライバを差し替えることはしない（システムを壊すリスクがある）。
...
これは memo.txt の 4.2「未知のシリアルデバイスへ無条件にデータを送信してはならない」
に対する技術的裏付けである。
...
7. **WCH-LinkE は WinUSB がバインドされていればアクセス可能。判定して案内する**（F6）
