...
測定環境: Windows 11 Pro 26200.9168 / usbipd-win 5.3.0 / WSL 2.7.12.0 (Ubuntu-24.04)

本書は仕様の前提となる事実を、測定した値とともに記す。
仕様そのものは [requirements.ja.md](requirements.ja.md) にある。
...
## F6. WCH-Link は純正ドライバのまま Windows アプリから開ける
...
この PC に接続したことのある WCH-Link は、interface 0 が **全数 `WCHLink_A64`**
（WCH 純正）である。Zadig / libwdi で WinUSB を割り当てた個体だけが `WinUSB` になる。

```text
oem79.inf   wchlinkwdm.inf              wch.cn    WCH 純正 WDM ドライバ
oem24.inf   wch-link_(interface_0).inf  libwdi    Zadig が割り当てた WinUSB
```

### 実測: 純正ドライバのままプローブできる

`ch32rv-usb` は、**nusb でインタフェースを claim できない場合に
CH375 系の `DeviceIoControl` 経路へ自動でフォールバックする。**
純正ドライバのままの WCH-Link に対して `ch32rv probe list` が
プローブ情報（`WCH-Link(CH549)` firmware 2.6）を返す。

### 結論

- **WinUSB の割り当ては前提条件ではない。** 純正ドライバのままプローブできる。
- したがって「Zadig で WinUSB を割り当ててください」という案内は行わない。
  ドライバの差し替えを求めれば、利用者に余計な操作を強い、
  かつ WCH-Link の COM ポート機能を失わせる。
- `DEVPKEY_Device_Service` の値は**表示のための情報**であり、
  プローブ可否の判定には用いない。
- WinUSB がバインドされている場合も、同じく管理者権限なしで開ける。
  WinUSB はユーザーモード API であり、libusb / nusb / pyusb は同じ経路を使う。
