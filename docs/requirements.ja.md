...
保存するのは**ユーザーが決めたこと**だけである。
アプリが観測した事実（列挙結果、識別結果、usbipd の状態）は保存しない。
次回の起動時にはもう一度観測すればよく、
...
**要件 R7.8**: プローブで得た Application Identity（§3.4）を保存してはならない。
識別結果は実行中のみ保持し、切断で破棄する（R4.2 / R4.3）。
...
**要件 R9.1**: デバイスごとに自動 Attach を設定できること。
...
**要件 R9.3**: 自動 Attach の対象は User Metadata のキー（R7.7）で指定する。
ポートパスで指定した場合は、
**そのポートに挿したものが Attach される**ことを設定時に示す。
...
| Transport | USB Serial。無い場合はポートで区別している旨を示す |
| Target | 識別結果、または未識別（§4.2、R4.4） |
